...
class LookaheadBuilder():
	...
	def _construct_transition_boxes(self):
		...
		for d in range(1,self.lookahead.depth):  # 1~d-1
		...
		for d in range(1,self.lookahead.depth):
			if self.lookahead.layers[d].indices is not None:
				...
				# layers[d].pot_size - [A{d-1}, B{d-2}, NTNAN{d-2}, b, P, I]
				pot_sizes = self.lookahead.layers[d].pot_size[ 1, p_start:p_end, : , 0, 0, 0 ].reshape([-1])  # num_nonterminal_nonallin_parents
				...
				if d == 1:
					...
					num_root_node_bets = self.lookahead.layers[2].pot_size.shape[1]  # num_grandparent_bets
	...
	def construct_data_structures(self):
		...
		for d in range(2, self.lookahead.depth):  # 2~d-1
			# data structures [A{d-1}, B{d-2}, NTNAN{d-2}, b, P, I]
		...
		for d in range(1,self.lookahead.depth):  # 1~d-1
			...
			if d > 1 or self.lookahead.first_call_terminal:
				if self.lookahead.tree.street != constants.streets_count:  # 1~3
					before = self.lookahead.num_term_call_nodes
					num_nonterminal_nonallin_grandparents = layers[d].ranges[1][-1].shape[0]  # term_call, allin
	...
	def set_datastructures_from_tree_dfs(self, node, depth, action_id, parent_id, gp_id, cur_action_id, parent_action_id=None):
		...
		if node.current_player == constants.players.chance:
			...
			assert(parent_id <= num_nonallin_bets)  # parent is not an allin raise
		if depth < self.lookahead.depth + 1:  # 0~d
			...
			next_parent_id = action_id - p_num_terminal_actions  # start from 0
			...
			if (not node.terminal) and (node.current_player != constants.players.chance):
				...
				if len(node.children) < self.lookahead.layers[depth].num_actions:
					...
					for action_id in range(len(node.children)): # go deeper, include allin
						next_node, next_action = node.children[action_id], node.actions[action_id]
						self.set_datastructures_from_tree_dfs(next_node, depth+1, action_id, next_parent_id, next_gp_id, next_action, cur_action_id)
	...
	def build_from_tree(self, tree):
		...
		for d in range(0,self.lookahead.depth+1):  # 0~d
	...
	def _compute_tree_structures(self, current_layer, current_depth):
		...
		self.lookahead.layers[current_depth].num_bets = layer_num_actions - layer_num_terminal_actions  # *check + raise(+allin)
		...
		self.lookahead.layers[current_depth].num_actions = layer_num_actions  # all_actions
		if len(next_layer) > 0:
